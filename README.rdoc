...
=== Financial Data Loaded in a Rails App
...
First, notice that I called the class CSV::ET, instead of CSV::ETL.  That's because I didn't implement any load function.  Also, notice that I required etl/csv_et explicitly.  There are a number of utilities that I've written.  More will be extracted and made useful over time:

* There is a CSV::ActiveRecordLoad that uses ar-extension's import function to load quickly into MySQL databases.  
* There is an XML::ET that works like CSV::ET, but with libxml instead of FasterCSV
* There is a XML::ActiveRecordLoad that works like CSV::ActiveRecordLoad
...
* Integrate some of the transforms made available in ActiveWarehouse-ETL.  That is a very good tool specialized to data warehousing.
